...
- **The canonical/OG/sitemap domain comes from `VITE_SITE_URL`** (set in `.env`), not a hardcoded string. `index.html` uses Vite's `%VITE_SITE_URL%` HTML env-replacement for `<link rel="canonical">`, `og:url` and `og:image`; `robots.txt` and `sitemap.xml` no longer live in `public/` — they're generated into `dist/` at build time by the `site-files` plugin in `vite.config.ts` from the same variable. To change the deploy domain, edit `.env` only.
- **`npm run build` prerenders the page** (`tsc -b && vite build && node scripts/prerender.js`): after the normal client build, `scripts/prerender.js` does a second Node-side SSR build of `src/entry-server.tsx` (`renderToString(<App/>)`) and bakes the result into `dist/index.html`'s `#root` div, so crawlers see real markup instead of an empty div. `src/main.tsx` then `hydrateRoot`s if `#root` already has children, or `createRoot`s otherwise (dev server, which always serves an empty root). Because of this, **nothing in `App.tsx` or its children may touch browser-only APIs outside of `useEffect`/event handlers** — that's already true today (`AudioEngine` is constructed lazily on user gesture, canvas/`window`/`matchMedia` access lives in effects) and must stay true for any future change to the landing view.
